...
class MinuteMETsLSTM(nn.Module):
...
def create_sequences(features: np.ndarray, window_size: int):
    """(N, F) → (N-window, window, F), label은 다음 시점의 features[:, 0](METs)."""
    seqs, labels = [], []
    for i in range(len(features) - window_size):
        seqs.append(features[i : i + window_size])
        labels.append(features[i + window_size, 0])  # 0번 컬럼이 METs
    return np.stack(seqs), np.asarray(labels)

def load_and_prepare(csv_path: str, window_size: int, batch_size):
    EPS = 1e-6
    df = pd.read_csv(csv_path)
    df["ActivityMinute"] = pd.to_datetime(
        df["ActivityMinute"], format="%m/%d/%Y %I:%M:%S %p", errors="coerce"
    )
    df["METs"] = pd.to_numeric(df["METs"], errors="coerce")
    df = df.dropna(subset=["ActivityMinute", "METs"]).reset_index(drop=True)

    # 가장 샘플 많은 참가자 선택
    target_id = df["Id"].value_counts().idxmax()
    target = (
        df.loc[df["Id"] == target_id, ["ActivityMinute", "METs"]]
        ...
        .ffill()
        .dropna()
        .rename(columns={"METs": "METs"})
        .reset_index()
    )

    # 24시간 주기 특성
    minute_of_day = target["ActivityMinute"].dt.hour * 60 + target["ActivityMinute"].dt.minute
    target["sin_time"] = np.sin(2 * np.pi * minute_of_day / 1440)
    target["cos_time"] = np.cos(2 * np.pi * minute_of_day / 1440)

    feature_cols = ["METs", "sin_time", "cos_time"]
    features = target[feature_cols].to_numpy(dtype=np.float32)

    # 슬라이딩 윈도우
    X_all, y_all = create_sequences(features, window_size=window_size)
    total = X_all.shape[0]
    tr_end = int(total * 0.7)
    va_end = int(total * 0.85)

    # 텐서 변환 & 분할
    X_train = torch.tensor(X_all[:tr_end], dtype=torch.float32)
    y_train = torch.tensor(y_all[:tr_end], dtype=torch.float32)
    X_val   = torch.tensor(X_all[tr_end:va_end], dtype=torch.float32)
    y_val   = torch.tensor(y_all[tr_end:va_end], dtype=torch.float32)
    X_test  = torch.tensor(X_all[va_end:], dtype=torch.float32)
    y_test  = torch.tensor(y_all[va_end:], dtype=torch.float32)
...
WINDOW_SIZE = 30
CSV_PATH = "/home/ubuntu/isfolder/fl_agent_paper/buildmodel/content/minuteMETsNarrow_merged.csv"
# CSV_PATH = "/app/code/data/minuteMETsNarrow_merged.csv" # for server
batch_size = 32
